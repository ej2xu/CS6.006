...
class PathFinder(object):
    ...
    def dijkstra(self, weight, nodes, source, destination):
        ...
        """
        for node in nodes:
            node.parentf, node.parentb, node.keyf, node.keyb = None, None, None, None
        visited = 0
        forward = PriorityQueue()
        backward = PriorityQueue()
        source.keyf, destination.keyb  = NodeDistancePair(source, 0), NodeDistancePair(destination, 0)
        forward.insert(source.keyf)
        backward.insert(destination.keyb)
        count = 1
        fkeys, bkeys = [], []
        while len(forward)>0 and len(backward)>0:
            keyf = forward.extract_min()
            keyb = backward.extract_min()
            fkeys.append(keyf)
            bkeys.append(keyb)
            nodef, distf = keyf.node, keyf.distance
            nodeb, distb = keyb.node, keyb.distance
            visited += 2
            if nodef == nodeb: break
            for node in nodef.adj:
                nextd = distf+weight(nodef, node)
                if node.keyf is None:
                    node.keyf = NodeDistancePair(node, nextd)
                    forward.insert(node.keyf)
                    node.parentf = nodef
                elif nextd < node.keyf.distance:
                    node.keyf.distance = nextd
                    forward.decrease_key(node.keyf)
                    node.parentf = nodef
            for node in nodeb.adj:
                nextd = distb+weight(nodeb, node)
                if node.keyb is None:
                    node.keyb = NodeDistancePair(node, nextd)
                    backward.insert(node.keyb)
                    node.parentb = nodeb
                elif nextd < node.keyb.distance:
                    node.keyb.distance = nextd
                    backward.decrease_key(node.keyb)
                    node.parentb = nodeb
        min = keyf.distance + keyb.distance
        minnode = nodef
        for fkey in fkeys:
            backkey = fkey.node.keyb
            if backkey is not None:
                if fkey.distance + backkey.distance < min:
                    min = fkey.distance + backkey.distance
                    minnode = fkey.node
        for bkey in bkeys:
            forkey = bkey.node.keyf
            if forkey is not None:
                if bkey.distance + forkey.distance < min:
                    min = bkey.distance + forkey.distance
                    minnode = bkey.node
        p = []
        nodeb = minnode
        while minnode is not None:
            p.append(minnode)
            minnode = minnode.parentf
        p.reverse()
        nodeb = nodeb.parentb
